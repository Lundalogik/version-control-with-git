...
Furthermore, any operation you do is logged by git. So even if you remove a reference to a commit from the structure of references, your running of that command, *complete with a reference to the commit* is stored in git's log.

This means that while you may accidentally "hide" a commit by deleting the reference to it, it's really hard to actually get rid of every reference to it, and it's not something you will happen to do by accident. So don't worry :)
