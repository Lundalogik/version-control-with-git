...
Since this is for demo-purposes, this whole thing isn't very complex, and we wouldn't normally mind adding this whole thing in a single commit, but let's use our imagination shall we?

If you run `git log`, you'll see that the last commit added "some methods". These are the methods concerning the door and the camouflage. Let's imagine these were actually rather large additions, and we might want to have one commit for the door-related changes, and another one for the camo-related changes.
...
Run `git add -p` and add only the changes relating to the door.

Now commit those changes with a relevant commit message.

Then add the camo-related changes. By the way. It looks like we added an unnecessary line-break in the `travelTo` method. Don't add that.
...
Either way, you'll want to not change the history of the master branch, as that's the most important branch in the repo, and you really want to minimise the risk of any headaches there.
