...
A typical example would be that someone fixed a bug in the master branch, but you still have a branch for supporting an older version of the software, and the same bugfix is needed there.
...
A merge conflict happens when you are trying to add two different changes to the same file, and git can't figure out how to reconsile the differences. For example, you might be rebasing your branch onto the latest version of `master`. You've made some changes in `some-file.js`. Your changes where based on what the file looked like when you created your branch. But since then, someone else made a change to the same file, and that change is now on `master`. When you rebase your branch onto `master`, git will first remove all your commits. Then all new commits from `master` will be added. And then git will add all your commits again, one by one. But when git tries to add your commit with the change in `some-file.js`, it detects that the way the file looked before your changes doesn't match the way it looks right now.
...
The commit we want has a bugfix for the following code:
...
The fix is in the branch `exercises/cherry-pick-bugfix`. Find out which commit contains the bugfix, and cherry-pick that commit into this branch. You will get a merge conflict which you will have to resolve.
